...
class ddpg_agent:
    def __init__(self, args, env, env_params):
        ...
        # create the replay buffer for manipulate
        self.bufferManipulate = replay_buffer(self.env_params, self.args.buffer_size, self.her_module.sample_her_transitions)
    ...
    def learn(self):
        ...
        # while (approach isn't trained)) 1 epoch -> 50 steps e2e 
                ...
                for _ in range(self.args.num_rollouts_per_mpi):  # 2
                    ...
                    while len(ep_obs) < 50:
                        ...
                        while np.linalg.norm(grip_pos - object_pos_goal) >= 0.031 and timeStep <= 20:
                            ...
                            #print(timeStep, np.linalg.norm(grip_pos - objectPos))
                     

                            if len(ep_obs) > 49:
                                break

                            #print('timestep: {}, np.linalg.norm(object_oriented_goal): {}'.format(timeStep, np.linalg.norm(object_oriented_goal)))
